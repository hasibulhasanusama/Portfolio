export default async function handler(req, res) {
    ...
    if (!apiKey) {
        return res.status(200).json({ error: 'GEMINI_API_KEY is missing in Vercel Environment Variables.' });
    }
...
7. Only provide information related to this portfolio.
    ...
    try {
        ...
        if (data.candidates && data.candidates[0]?.content?.parts[0]?.text) {
            ...
            return res.status(200).json({ error: "Couldn't generate a response." });
        }
    } catch (error) {
        return res.status(200).json({ error: `Server Error: ${error.message}` });
    }
}